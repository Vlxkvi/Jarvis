...
async function autoNews(messageContent, client){
    try {
        const notificationChannel = await client.channels.fetch('1103310828104597534');
        const Titles = {
          'FREE VEHICLES': 'empty',
          'NEW CONTENT': 'empty',
          'BONUS': 'empty',
        ...
        if (notFoundTitles.length !== 0) {
          ...
          notificationChannel.send(notFoundMessage);
      }
      
    } catch (error) {
        console.error(error);
        notificationChannel.send({ content: `Seems you're trying to load message which is not from <#795155910153469952> or we have some other error. Anyway tell that to <@163547278882111488> if you have this problem few times`, ephemeral: true });
    }
}
